			...
			if (acao != null && !acao.isEmpty() && acao.equalsIgnoreCase("deletar")) {
				...
				if (modelLogin.getFotoUser() != null && !modelLogin.getFotoUser().isEmpty()) {
					response.setHeader("Content-Disposition",
							"attachment;filename=foto_usuario_" + idUser + "." + modelLogin.getExtesaoFotoUser());
					response.getOutputStream()
							.write(Base64.getDecoder().decode(modelLogin.getFotoUser().split(",")[1]));
				}
				...
				if (dataInicio == null || dataInicio.isEmpty() && dataFim == null || dataFim.isEmpty()) {
					...
					modelLogins = daoUsuarioRepository.consultaUsuarioListRelData(super.getUserLogado(request),
							dataInicio, dataFim);
		...
		try {
			...
			if (request.getPart("fileFoto") != null) {
				...
				if (part.getSize() > 0) {
					...
					/*
					 * Essa concatenação gera uma string no formato correto para ser usada
					 * diretamente em atributos src de imagens no HTML. Esse formato é ideal para
					 * armazenar ou exibir imagens diretamente no navegador
					 */
					 ...
					 * O código split("\\/")[1] divide essa string pelo caractere '/' e pega a
					 * segunda parte
					 */
